...
public class RepeatController {
	public void basicRepeat() {
		//기본 for문 이용하기
		//특정코드를 원하는 만큼 반복해서 실행하게 하는 명령어
		//for(초기식;조건식;증감식) {반복할 코드를 작성}
		//초기식 : 변수선언, 반복한 횟수에 대해 저장하는 공간
		//조건식 : 반복을 더 실행하지말지 결정하는 조건(true실행,false중단)
...
//		String friend = ""; //★★ 변수를 누적하여 출력할때 변수는 밖에 생성해야한다.
//		for(int i = 1; i <= 5; i++) {
...
//		}
		...
		public void emInputData() {
			...
			for(int i = 0; i < count; i++) {
				...
				empInfo += i+1 + "번 사원정보 " + name + " " + age + " " + gender + " " + salary + " " + bonus + "\n";
			}
...
//				System.out.println(i);
		...
		public void check() {
			...
			for(int i = 0;i<data.length(); i++) {
				...
				//else{flag = false;} else구문이 있으면 if구문이 무조건 실행되므로 버그발생!!
				
				}
			System.out.println(flag?"대문자 있어!" : "대문자없어!");
		}//check()			
			//입력받은 문자열에 대문자와 소문자의 수를 구하능 기능
		...
		public void whileCalculator() {
			...
			do {	
				...
				String cal = sc.next();
				switch(cal) {
				case "+" : System.out.println(num1+cal+num2+"="+(num1+num2)); break;
				case "-" : System.out.println(num1+cal+num2+"="+(num1-num2)); break;
				case "*" : System.out.println(num1+cal+num2+"="+(num1*num2)); break;
				case "/" : System.out.println(num1+cal+num2+"="+((double)num1/num2)); break;
				case "&" : System.out.println(num1+cal+num2+"="+(num1%num2)); break;
				default : System.out.println("연산자를 잘못 입력했습니다.");break;
				}
...
//		}
		//랜덤값을 정수형으로 출력하기
		//Math.random()*3 -> 0~2까지의 랜덤값 출력
		//Math.random()*10 -> 0~9까지의 랜덤값 출력
		//Math.random()*20 -> 0~19까지의 랜덤값 출력
		...
		System.out.println(r.nextInt(10)); //0~9까지의 수
		System.out.println(r.nextInt(20)); //0~19까지의 수
		System.out.println(r.nextInt(10)+1); //1~10까지의 수
	}//randomTest();
