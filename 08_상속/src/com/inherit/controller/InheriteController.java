...
public class InheriteController {
...
//		double test=weight; <<weight는 protected로 선언되있는 변수라서 상속을 받지 않으면 사용 불가능
		...
		Student joo=new Student("최주영",26,'남',"010111","경기도 안양시","공공데이터", "선생을 갈굼",'상');

		System.out.println(joo.infoStudent());
		
		Student jung=new Student("정상준",26,'남',"010222","서울시 구로구","공공데이터", "힘들어함",'중');

		System.out.println(jung.infoStudent());
		
		//선생 1명
		//유병승, 19, 남, 0101234,경기도 시흥시, 웹프로그래밍,100,100
		
		Teacher te=new Teacher("유병승",19,'남',"0101234","경기도 시흥시","웹프로그래밍",100,100);
		//t.infoPerson(); 사용할수 없는이유는 Person 클래스에  
		//infoPerson 이란 메소드를 protected로 만들어서 상속하지 않으면호출불가
		System.out.println(te.infoTeacher());
	...
	public void overrideTest() {
		...
		//toString : 생성된 객체의 대표되는 문자를 반환(필드의 값을 반환) 
		//clone : 생성된 객체를 복사해주는 기능
		
		//equals 재정의 하기
		Student s1=new Student("최주영",26,'남',"010111",
					"경기도 안양시","공공데이터", "선생을 갈굼",'상');
		Student s2=new Student("최주영",26,'남',"010111",
					"경기도 안양시","공공데이터", "선생을 갈굼",'상');
		Student s3=new Student("정상준",26,'남',"010222",
					"서울시 구로구","공공데이터", "힘들어함",'중');
		...
		//자동으로 toString()메소드를 호출하여 지정된 정보를 출력
	...
	if(t.equals(t1)) {
		System.out.println("t와 t1은 같은객체입니다.");
	}else if(t.equals(t3)){
		System.out.println("t와 t3는 같은객체입니다.");
	}else if(t1.equals(t3)) {
		System.out.println("t1와 t3같은객체입니다.");
	}else {
		System.out.println("같은 객체는 없습니다.");
	}
